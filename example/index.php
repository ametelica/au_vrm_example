...
if($act == 'options') {
    ...
    $responce = [
        ...
        'vars' => [                     // переменные, которые можно будет настроить в блоке, будут видны в виде полей с
            'search' => [               // разными типами, по умолчанию text
            ...
            'author' => [
                ...
                'values' => [           // Для этого достаточно определить values
                    0 => 'Исполнитель',
                    1 => 'Автор'
                ]
            ],
            'add_fld' => [
                'title' => 'Показывать доп. поля',      // Пример поля с выпадающим списком
                'values' => [
                    0 => 'Нет',
                    1 => 'Да',
                    2 => 'Может быть'
            ...
            'active' => [               // Поле - переключатель
                'title' => 'Активность',
                'format' => 'checkbox',
                'values' => [           // Подписи к позициям выключителя, не обязательно, без них будет просто ДА и НЕТ
                    0 => 'Нет!',
                    1 => 'Да!',
                ]
            ],
            'date_start' => [            // Поле с выбором даты и времени
                'title' => 'Когда начать',
                'format' => 'datetime',
            ],
            'author_level' => [
                'title' => 'Уровень автора',
                'show' => [             // Это поле должно показываться только если author = 1 и add_fld 1 или 2
                    'author' => 1,
                    'add_fld' => [1, 2]
                ]
            ],
        ...
        'paysys' => [                   // Группа полей, отвечающая за интеграцию с платёжными системами и внешними сервисами.
            'ps' => [                   // ВРМ получит доступ к ID аккаунта, секретному ключу и другим атрибутам выбранной системы
                ...
                'type' => 1             // Ограничим выбор сервисов только одним типом: 1 Яндекс, 2 VKPay, 3 Яндекс.Касса, 4 GetCourse, 5 Продамус, 6 Senler
            ]
        ],
    ...
    $responce = [
        'html' => 'Инструкция, как пользоваться этой ВРМ'
    ];
